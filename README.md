...
#### v1.3.16（2026/06/04）
* Privacy Blur の再非アクティブ化漏れを修正
設定 -> 一般 -> Privacy で Blur が ON の状態において、プレビュー画面をクリックしたあとにタブが非アクティブ化された場合でも、正しくブラーが掛かるように修正しました。

* 再現テストを追加して回帰を防止
「preview クリック後に非アクティブ化したらブラーが掛かる」ケースをユニットテストへ追加し、同不具合の再発を防止しました。
